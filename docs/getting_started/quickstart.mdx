...
<AccordionGroup>
  ...
  <Accordion title="Step 2: Download MemMachine and Run the Script">
  ...
  You will need to create your [Neo4j](http://hub.docker.com/_/neo4j) and [PostgreSQL](https://hub.docker.com/_/postgres) databases before starting MemMachine, as MemMachine requires information from these databases for it's configuration files.
   <Steps> 
    <Step title="Start Neo4j">
     Check out the [Neo4j Docker Hub](http://hub.docker.com/_/neo4j) page for installation instructions.  Make sure your Neo4j instance is configured with the following settings:
      ```yaml
       vendor_name: neo4j
        host: localhost # If using docker-compose, host: neo4j
        port: 7687
        user: neo4j
    ...
    For session management, you can use PostgreSQL.  To install PostgreSQL, check out the [PostgreSQL Docker Hub](https://hub.docker.com/_/postgres) page.  Then, follow these steps to configure your PostgreSQL database:
     ```sql
...
<Accordion title="Step 5: Start MemMachine">
Before your very first run, you'll need to sync the profile schema. Make sure you run this command from the directory where you installed memmachine.
  ```bash
...
If you have run MemMachine before, start the server with:
  ```bash
  docker run \
  --name neo4j \
  -p7474:7474 -p7687:7687 \
