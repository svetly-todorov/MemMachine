...
# Configuration
API_URL = os.getenv("API_URL", "http://memmachine:8080/api/v2/memories")
...
def get_dropbox_path() -> Path:
    """Return python3 path for the dropbox dir."""
    dropbox_dir_path = Path("/dropbox")
    if not dropbox_dir_path.exists():
        raise OSError("/dropbox does not exist")
...
def forward_api_v2_memories(msg_file: Path, api_url: str) -> bool:
...
def add_new_messages():
    ...
    for f in dropbox_dir_path.iterdir():
        ...
        host = str(f.name)
        json_requests = list[Any](f.glob("*.json"))
        ...
        for json_request in json_requests:
            file_timestamp = extract_timestamp_from_filename(json_request)
            ...
            if compare_timestamps(file_timestamp, last_saved_timestamp):
                if not forward_api_v2_memories(json_request, API_URL):
                    log_message(f"Error: Failed to forward message from {json_request}")
                    return

            ## Update most_recent_timestamp if file_timestamp is greater
            ## Done seperately from previous timestamp in case the json_requests are not processed in order
