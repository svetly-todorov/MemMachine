...
class SlackService:
    ...
    async def get_user_display_name(self, user_id: str) -> str:
        ...
        try:
            ...
            logger.error(f"[SLACK] users.info failed: {e.response['error'] if hasattr(e, 'response') else str(e)}")
            return user_id
        except Exception as e:
            logger.exception("[SLACK] users.info unexpected error")
            return user_id

    async def post_message(self, channel: str, text: str, thread_ts: Optional[str] = None) -> Optional[str]:
        """
        Post a message to a channel (optionally in a thread). Returns ts if successful.
        """
        try:
            resp = await self.client.chat_postMessage(channel=channel, text=text, thread_ts=thread_ts, mrkdwn=True)
            return resp.get("ts")
        except SlackApiError as e:
            logger.error(f"[SLACK] chat.postMessage failed: {e.response['error'] if hasattr(e, 'response') else str(e)}")
            return None
        except Exception as e:
